...
Module("GabTech.Collapsible", function(Collapsible){
	Collapsible.fn.initialize = function($container, $closeOthers){
		this.container = $container;
		this.titulo = $container.find('.titulo');
		this.conteudo = $container.find('.conteudo');
		this.trocaCor = $container.hasClass('fechado-cinza');
		this.closeOthers = $closeOthers;

		$container.hasClass('aberto')
			&& $container.removeClass('fechado-cinza');
	...
	*/
	Collapsible.fn.toggleOpen = function(){
		this.conteudo.slideToggle('500');
		this.container.toggleClass('aberto');

		if(this.trocaCor){
			this.container.toggleClass('fechado-cinza');
		}

		if( this.closeOthers){
			this.titulo.parent().siblings('.collapse.aberto')
				.find('.conteudo').slideUp().end().removeClass('aberto');
		}
		
		return false;
	};
});
Module('GabTech.Mascarar', function(Mascarar){
	...
	*/
	Mascarar.fn.config = function(){
		this.inputs
			...
			.filter('.mask-real').maskMoney({prefix:'R$ ', allowNegative: true, thousands:'.', decimal:',', affixesStay: true}).end();

		this.inputs.filter(function(index) {
			var _this = $(this);
			
			if( _this.hasClass('mask-personalizado') ) {
				_this.mask(_this.data('mascara')).data('mask', _this.data('mascara').replace(/9/g, '_'));
			}
		});
	};
});
Module("GabTech.Listas", function(Listas){
...
Module('GabTech.MenuResponsivo', function(MenuResponsivo){
	...
	MenuResponsivo.fn.config = function(){
		...
		$(window).resize(function(event) {
			...
			if( $(window).width() > 1290 ){
				...
				this.botao.add($('a[title=GabTech]', this.menu).stop(0, 0)).stop(0, 0).fadeToggle();
				...
				!$('#global').hasClass('menu-aberto') && $('li a span, .usuario span', _this.menu).stop(0, 0).animate({opacity: 0}, 300, function(){
...
Module('GabTech.FileUpload', function(FileUpload){
...
Module('GabTech.ValidarFormularios', function(ValidarFormularios){
			...
			showErrors: function(errors){
				...
				for (var i = $elem.length - 1; i >= 0; i--) {
					if( $( $elem[i] ).hasClass("validado") ){
						$elem.parent().addClass('validado').removeClass('verificando');
					}
...
Module('GabTech.VerificarNavegador', function(VerificarNavegador){
...
var site = {
	...
	*/
	global: function(){
		GabTech.MenuResponsivo !== undefined && GabTech.MenuResponsivo();
		GabTech.Mascarar !== undefined && GabTech.Mascarar();
		if( $('div.collapse').length ){
			$('div.collapse').each(function(index, el) {
				GabTech.Collapsible($(this), true);
			});
		}
		GabTech.Listas !== undefined && GabTech.Listas();
		$('form.validar').each(function(index, el) {
			var obj_temp = $(el).data('settings'),
				callback  = $(el).data('callfn');

			GabTech.ValidarFormularios($(el), obj_temp, callback);
		});
		if ($('main.cadastro').length) {
			this.global_cadastro();
		};

		GabTech.VerificarNavegador();
	},
	/*
	* Funcionalidade da página de LOGIN. Ajuste da altura
	*/
	login: function(){
		$('.login, .login .unicelular > .linha').height($(window).height());
	...
	/*
	* Funcionalidades que pode ser usada em todos os formulários do projeto.
	*/
	global_cadastro: function(){
		$('main').on('click', '.btn-novoCampo', function(event) {
			var _btn = $(this),
				_parent = _btn.parent(),
				_cloneField = $('input:first', _parent).clone().val(''),
				_bloco = $( '<div class="novo"> <a href="#this" class="btn-deletarNovo" title="Excluir"><i class="icon icon-excluir"></i></a></div>' );

			_cloneField.prependTo(_bloco);

			_bloco.insertBefore(_btn);
			GabTech.Mascarar( _bloco.find('input') );
		})
		.on('click', '.btn-editarView', function(event) {
			var _btn = $(this),
				_viewPreenchida = _btn.parents(".view-preenchida");

			_viewPreenchida.fadeOut(function() {
				_viewPreenchida.prev('.view-oculta').fadeIn(function() {
					$(this).removeClass('view-oculta').addClass('view-ativa');
				});
			});
		})
		.on('click', '.btn-deletarNovo', function(event) {
	...
	/*
	* Funcionalidade de troca de paddo na página de Cadastro.
	*/
	cadastro_troca_passo: function($formulario, $resposta){
		var _viewativa = $('.view-ativa')

		_viewativa.fadeOut(function() {
			_viewativa.removeClass('view-ativa').addClass('view-oculta');
			if( _viewativa.next('.view-vazia').length ){
				_viewativa.next('.view-vazia').fadeOut(function() {
					$(this).next('.view-oculta').fadeIn(function() {
						$(this).removeClass('view-oculta').addClass('view-ativa');
						$('html, body').animate({scrollTop:$(this).offset().top}, 'slow');
					});
				});
			}
			$($resposta).insertAfter(_viewativa);
		});
	}
}
