!function($) {
  ...
  Draggable.prototype.start = function(e) {
    ...
    $(this.el).addClass(this.opts.clsMove); // Adicionando a classe quando começa o DRAG
    ...
    if (this.opts.cancel) {
      ...
      while (target[0] !== this.el[0]) {
        if (target.is(this.opts.cancel)){

          console.log(target);
          $(this.el).removeClass(this.opts.clsMove);
          return false
        };
  ...
  Droppable.prototype.drop = function(e, originalEvent) {
    ...
    if( this.opts.callback ){
      this.opts.callback(this, dragging);
    }
